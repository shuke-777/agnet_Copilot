...
### M17：客户 360 与客服待办工作台（下一阶段）

```text
侧边栏新增“待办中心”和“客户360”两个独立入口，与 Copilot 工作台、工单中心、Agent 追踪、运营看板和操作日志并列；客服默认先从待办中心进入，再按工单进入客户360查看上下文
第一版定位为客服处理前的客户总览和待办入口，不替代工单中心，也不直接执行退款、补发、补偿等高风险动作
页面支持按订单号、用户 ID 或后续手机号/昵称检索客户上下文；当前阶段优先复用本地已有 orders、logistics、tickets、ticket_events、agent_runs 和 operation_logs 数据
首屏展示客户基本信息、最近订单、物流状态、最近工单、风险标签和当前待处理事项
详情区展示物流轨迹、工单事件时间线、关联 Agent Run、历史操作记录和客服备注入口
待办队列支持按状态、优先级、处理人和是否超时筛选；客服可以认领、处理、解决和重新打开工单
第一版 SLA 使用固定处理时限：高优先级工单 30 分钟内响应，普通工单 2 小时内响应；SLA 从工单创建时间开始计时，客服认领不重置计时；截止时间、剩余时间和超时状态在队列及工单详情中可见
工单进入 resolved 或 closed 后停止 SLA 计时；首次超时写入 ticket_events 和 operation_logs，并在待办队列和运营看板中显示风险标记
右侧动作区提供新建工单、跳转订单/物流、跳转工单详情、查看 Agent Trace、添加客服备注等低风险操作
客户 360 不在第一版接真实电商平台、不新增真实客户隐私字段、不引入登录/权限体系；这些能力在真实平台接入和多租户阶段单独规划
验收场景：客服从侧栏进入待办队列，看到 ORD-1001 的异常物流工单及 SLA 倒计时；点击进入客户 360 后能看到订单、异常物流、关联工单、处理事件、Agent Run 和操作日志，认领并解决工单后 SLA 停止计时且全程可追溯
```
