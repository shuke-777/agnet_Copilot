...
- 当前优先级：M17 客户 360 与客服待办工作台 -> M11 RabbitMQ 可靠投递。
- M17：侧边栏新增“待办中心”和“客户360”两个独立入口；客服默认先从待办中心进入，再按工单进入客户360。第一版复用本地已有订单、物流、工单、Agent Run 和操作日志数据，展示客户基本信息、最近订单、物流状态、最近工单、风险标签、处理事件和跳转入口。
- M17 SLA：高优先级工单默认 30 分钟内响应，普通工单默认 2 小时内响应；SLA 从工单创建时间开始计时，客服认领不重置计时；队列和工单详情展示截止时间、剩余时间和超时状态，工单解决或关闭后停止计时，首次超时写入工单事件和操作日志。
- M17 第一版不接真实电商平台、不新增真实客户隐私字段、不引入登录/权限体系；售后执行动作、真实平台回写和多租户权限后续单独规划。
- M17 第一批实现已完成：新增 `GET /api/todos` 和 `GET /api/customers/{user_id}/360`，工单响应带 SLA 截止时间、剩余秒数和超时标记；新增“待办中心”和“客户360” React 页面，支持 `ORD-1001` / `USER-001` 本地演示闭环。
- M17 第二批已完成：客户 360 支持按订单号检索，并返回按时间倒序聚合的订单、工单、工单事件、Agent Run 与操作日志时间线；前端已展示订单号筛选和处理时间线。
- M17 自动验证已完成：后端 125 passed + M17 专项新增场景通过，前端 16 passed，前端生产构建通过；后续补充浏览器人工验收截图和 SLA 超时统计看板。
